...
const API = {
  ...
  async getPlayerPriceHistory(playerSlug, rarity, opts = {}) {
    ...
    while (hasMore) {
      ...
      console.log(`[API] paginate: fetched ${allRows.length} rows so far (has_more=${hasMore})`);
    }

    const sales = allRows.map(r => this.normalizeSale(r)).reverse();
    return sales;
  },
  ...
  async searchPlayers(query) {
    ...
    try {
      ...
      return (data.players || data).map(row => ({
          slug: row.player_slug || row.slug,
          name: row.player_display_name || row.name || '',
          team: row.team_name || row.team || '',
          position: Array.isArray(row.positions) ? row.positions.join('/') : (row.positions || row.position || ''),
          pictureUrl: row.picture_url || row.pictureUrl || null
      })).filter(p => MLS_TEAM_NAMES.has(p.team)).slice(0, 10);
    } catch (err) {
